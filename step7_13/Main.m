...
save("PH_13.mat","PH");
save("cost_13.mat","cost");
save("Pe.mat","Pe");
trade = sdpvar(1,1);
f = -(U_no(1)-Obj_MG1(iter)+trade)*(U_no(3)-Obj_MG3(iter)-trade);
con = [U_no(1)-Obj_MG1(iter)+trade>=0;
    U_no(3)-Obj_MG3(iter)-trade>=0;];
ops = sdpsettings('solver','cplex','verbose',0);

result = optimize(con, f, ops);

if result.problem == 0     
    a = value(Obj_MG1(iter)-trade)
