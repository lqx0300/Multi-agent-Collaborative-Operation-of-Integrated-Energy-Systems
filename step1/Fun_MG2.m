...
L_h0=[1610,1594,1594,1610,1633,1633,1286,1201,1117,1109,1648,1656,1664,1140,1124,1109,1286,1309,1302,1325,1479,1502,1340,1332];
Predict_pv=[0,0,0,0,0,0,967,1287,1583,1833,1918,1942,2004,1957,1669,1076,655,0,0,0,0,0,0,0]*3;
...
%% 目标函数
Obj=sum(pri_e.*P_buy)+2.5*sum(Gas)+0.01*sum(abs(P_e_tran))-0.03*sum(P_e_cut)-sum(grid_sw.*P_sell)+C3...
    +0.016*sum(P_h_DR);    
%% 求解器配置与求解
...
if result.problem == 0     
    v2 = double(Obj)
    c1 = value(sum(pri_e.*P_buy))
    c2 = value(2.5*sum(Gas))
    c3 = value(sum(grid_sw.*P_sell))
    c4 = c1+c2-c3
    c5 = value(0.01*sum(abs(P_e_tran))-0.03*sum(P_e_cut)+0.016*sum(P_h_DR))
    c6 = value(C3)
else
 disp('求解失败，失败原因为：')
 disp(result.info)
