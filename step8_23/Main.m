...
while 1
    ...
    r = sum(norm(P_e_23(iter+1,:)-z_23(iter+1,:))^2)+sum(norm(P_e_21(iter+1,:)-z_21(iter+1,:))^2);
...
Obj_MG3(iter);
time = 1:24;
PH = [time;p1;h1;p2;h2;p3;h3];
cost = [c1;c2;c3];
Pe = [P_e_12(iter+1,:);P_e_13(iter+1,:);P_e_23(iter+1,:)];
save("PH_23.mat","PH");
save("cost_23.mat","cost");
save("Pe.mat","Pe");
trade = sdpvar(1,1);
f = -(U_no(3)-Obj_MG3(iter)+trade)*(U_no(2)-Obj_MG2(iter)-trade);
con = [U_no(3)-Obj_MG3(iter)+trade>=0;
    U_no(2)-Obj_MG2(iter)-trade>=0;];
...
if result.problem == 0     
    ...
    b = value(Obj_MG2(iter)+trade)
    c = value(Obj_MG3(iter)-trade)
else
 disp('求解失败，失败原因为：')
 disp(result.info)
end
